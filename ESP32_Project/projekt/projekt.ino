...
void loop() {
  ...
  if (millis() - lastDisplayUpdate >= displayUpdateInterval) {
    ...
    if (!intrusionAlarmActive && !waterAlarmActive && !gasAlarmActive) {
      if (isSystemArmed) {
          displayManager.triggerAlarm("SYSTEM ARMED");
      } else {
  ...
  if (motionManager.getIsEnabled() && motionManager.isMotion()) {
    if (isSystemArmed && !intrusionAlarmActive) {
        ...
        displayManager.triggerAlarm("MOTION!");
    }
  }
  ...
  if (rfidManager.loop()) {
    ...
    if (!isCardHeld) {
      cardStartTime = millis(); isCardHeld = true; actionExecuted = false;
